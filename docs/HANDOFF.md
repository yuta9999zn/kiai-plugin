...
| **Model bất kỳ** (Qwen, Llama, GPT qua API, LM Studio, vLLM…) | harness gọi `kiai wrap` cho mỗi tool call | **ĐÃ ĐO — có giới hạn** | `qwen2.5:7b` qua Ollama, 2026-09-18: `git reset --hard` viết trần **bị chặn**, model đọc lý do và tự giải thích lại; 6 record, `verify` xanh. **Nhưng** luật so khớp **chuỗi lệnh**: cùng lệnh viết bằng biến + `eval`, hay `git -c alias.x='reset --hard' x`, **đi lọt và phá dữ liệu thật** (đo 18/09, review vòng 1). Từ 0.8.0 hai cách viết ấy không còn `CLEAR`: luật `safety/an-unreadable-command-is-not-a-pass` **cảnh báo** "không đọc được lệnh" và ghi vào chuỗi (77/77 cách viết lách bắt — gồm dạng sinh đôi reviewer độc lập viết qua 4 vòng; 0/115 lệnh lành phần lớn của reviewer — vòng 1 từng 13/56 oan vì `export VAR="…"` và đường dẫn Windows tương đối, vòng 2 từng 7/7 trên `git config` thường dùng, vòng 3 từng 8/62 (`-File x.ps1 -e prod`, `core.sshCommand="ssh -i $HOME/…"`), vòng 4 từng 2/29 (`-Command (Get-Date)`); 0 oan trên mọi lệnh thật trong hộp đen KIAI, test tự đếm (223 lúc viết) — `test/opaque.test.mjs`); vẫn **không chặn**. Cái không vòng qua được là **snapshot** cây làm việc `wrap` chụp trước mỗi lệnh — xem §5 |
...
`init` tạo `.kiai/` **và gieo 27 luật khởi điểm** vào `.kiai/rules/` (đo 2026-09-18: bản cài từ marketplace trước đó có lệnh `rules` mà không có luật nào — cái khoá không có ổ). Rồi:
...
**Dây bẫy thứ hai (0.8.0, UOW-132):** op `opaque` phân tích shell ở mức tối thiểu — bóc vỏ `bash -lc '<literal>'` (cách harness bọc mọi lệnh) rồi hỏi *chuỗi này có nói được chương trình nào sẽ chạy không*. `eval`, biến/`$(…)` ở vị trí lệnh, `bash -c "$X"`, `… | sh`, `git -c alias.…`, tên ghép bằng dấu nháy ⇒ **WARNING** `safety/an-unreadable-command-is-not-a-pass` kèm lý do, **ghi vào chuỗi** như một `note`, lệnh **vẫn chạy**. Đo hai mặt: 77/77 cách viết lách (hai đòn trên, `Invoke-Expression`, `GIT_CONFIG_PARAMETERS=…alias.…`, `powershell -ExecutionPolicy Bypass -Command "$X"`, `timeout 30 bash -c "$X"`, `''gi''t`, `echo "don't $(eval "$X")"`, `bash <<EOF … EOF`, `f(){ eval "$X"; }; f`…) · 0/115 lệnh lành (phần lớn do reviewer độc lập viết — vòng 1 từng **13/56 oan**: `export VAR="…"`, `src\app.py`; vòng 2 từng **7/7** trên `git config` thường dùng; vòng 3 từng **8/62**: `-File x.ps1 -e prod`, `core.sshCommand="ssh -i $HOME/…"`; vòng 4 từng **2/29**: `-Command (Get-Date)`; corpus của tác giả không đo được mặt này — mỗi vòng reviewer lại tìm thêm) · 0 oan trên mọi lệnh thật trong hộp đen KIAI, test tự đếm (223 lúc viết; dương tính thật được liệt kê trong test kèm ai chạy, vì sao). Không thấy (17 ca ghim bằng test — là cái đã biết, không phải cái tồn tại): `git 'reset' --hard`, `git ${X:-reset}`, `git reset --ha""rd`, `rm -r""f`, `bash x.sh`, `bash -s < x.sh`, `python -c "…"`, `ssh host bash -c "$X"`, `docker run … sh -c "$X"`, `source "$F"`. Thứ **không** vòng qua được bằng cách viết lại lệnh: trước mỗi lệnh, `wrap` chụp **toàn bộ cây làm việc** (kể cả tệp chưa `git add`, theo `.gitignore`) thành một tree object trong `.git/objects` — ~90 ms. Nếu lệnh làm cây thay đổi, `wrap` ghi một `note` vào chuỗi kèm id tree và in ra stderr: `git restore --source=<tree> -- <tệp>` lấy lại tệp. `git gc` mặc định giữ object lơ lửng 2 tuần — **nhưng** `git gc --prune=now` hay `git prune --expire=now` xoá nó **ngay**, kể cả khi lệnh ấy chạy ở một lượt `wrap` **khác, sau đó**, và lệnh xoá luôn `.git` thì không còn gì để chụp (`wrap` nói rõ *"không xác định được cây có đổi hay không"* thay vì im lặng). Cả hai **chưa vá** — nợ N3. Tắt bằng `--no-snapshot`. **Đây là câu trả lời thật cho vụ `ollama.rb`: không phải "chặn được", mà là "lấy lại được" — trừ hai ca trên.**
...
- **Codex: không chặn được gì** — chỉ ghi sau. Cursor: **đã đo phiên sống** (§1, §6) — nhưng chỉ chặn được qua hook, không chặn được gì Cursor làm ngoài hook.
- **23/27 luật là `advice`** — tra cứu được, không cưỡng chế. `kiai rules list` in mức của từng luật.
- **Lệnh "không đọc được" chỉ được cảnh báo, không bị chặn** — và máy dò là danh sách đóng 7 lý do, không phải bộ phân tích shell đầy đủ. Một cách viết mới ngoài danh sách thì vẫn `CLEAR`; corpus trong test là giới hạn của lời khai.
