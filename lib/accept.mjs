...
import { sha256, cell } from './flight.mjs';
...
const L = {
  en: {
    ...
    howto3: 'Human-only decisions are a convention enforced by an environment check, not a cryptographic signature (planned).',
  ...
  vi: {
    ...
    howto3: 'Quy ước "chỉ người quyết" được cưỡng chế bằng kiểm biến môi trường, chưa phải chữ ký mật mã (dự kiến).',
...
export function collectUow(records, uow) {
  ...
  for (const r of scoped) {
    ...
    if (r.event === 'tool_call' && r.input) {
      if (r.input.file && r.input.content_sha256) {
        files.set(r.input.file, { path: r.input.file, sha256: r.input.content_sha256, bytes: r.input.content_bytes ?? null, kind: String(r.tool) === 'Write' ? 'write' : 'edit', outside: r.input.outside_repo === true, cwd: r.cwd ?? null, ts: r.ts, seq: r.seq });
      }
      if (typeof r.input.command === 'string' && r.input.command) {
        commands.push(r.input.command);
        if (isTestCommand(r.input.command)) {
          const res = r.tool_use_id ? results.get(r.tool_use_id) : null;
          const ok = res && res.result && typeof res.result.ok === 'boolean' ? res.result.ok : null;
          tests.push({ command: r.input.command, ok, ts: r.ts });
        }
      }
    }
...
export function renderPacket(pkg, lang = 'en') {
  ...
  if (pkg.files.length === 0) out.push(`_${t('filesNone')}_`);
    ...
    out.push('|---|---|---|---|');
    for (const f of pkg.files) out.push(`| ${code(f.path, 300)} | ${cell(f.kind, 10)} | ${code(String(f.sha256).slice(0, 16) + '…')} | ${diskLabel[f.disk] || cell(f.disk, 30)} |`);
  }
  out.push('');
...
export function checkPacket(md) {
  ...
  return { ok: actual === m[1], expected: m[1], actual };
}
