...
export function noteRecord(text, { cwd, now = new Date(), env = process.env, by = null, session = null, tool_use_id = null } = {}) {
  const workdir = cwd || process.cwd();
  const rec = {
    ts: now.toISOString(), event: 'note', agent: redact(String(by || env.KIAI_ACTOR || 'human')).slice(0, 80),
    // A note written about a tool call carries that call's session and id, so `report --session`
    // finds it and a reader can pair it with the tool_call (review 132 N1). A human note has neither.
    session: session == null ? null : String(session).slice(0, 120),
    cwd: workdir, uow: detectUow(workdir, env), text: redact(String(text)).slice(0, 4000),
  };
  if (tool_use_id != null) rec.tool_use_id = String(tool_use_id).slice(0, 120);
  return rec;
}
