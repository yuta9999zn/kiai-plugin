...
export function buildRecord(payload, { event, cwd, now = new Date(), env = process.env } = {}) {
...
export function summarize(records) {
  ...
  for (const r of records) {
    const key = r.session ?? (r.event === 'note' ? 'notes' : 'unknown');
    ...
    if (r.event === 'tool_call' && r.tool) {
      ...
      if (r.input && r.input.file && r.input.content_sha256) s.files.set(r.input.file, r.input.content_sha256);
